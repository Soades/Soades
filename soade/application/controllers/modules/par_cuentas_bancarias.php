...
class Par_cuentas_bancarias extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('par_cuentas_bancarias_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('par/view_par_cuentas_bancarias',$data);
	...
	{
		$data = array(
			'cb_idbanco' => $this->input->post('cb_idbanco'),
			'cb_idinstitucion' => $this->input->post('cb_idinstitucion'),
			'cb_nrocuenta' => $this->input->post('cb_nrocuenta'),
			'cb_tipo' => $this->input->post('cb_tipo'),
			 ); 

		$msn = array(); 

		$result=$this->par_cuentas_bancarias_model->create($data);
		$data['cb_idcb'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['cb_idcb'])) ? $_REQUEST['cb_idcb'] : '';
		$result=$this->par_cuentas_bancarias_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->par_cuentas_bancarias_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->par_cuentas_bancarias_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('cb_idcb');

		$result=$this->par_cuentas_bancarias_model->update($id,$data);
		$data['cb_idcb'] = $id;
	...
	function delete()
	{
		...
		$result=$this->par_cuentas_bancarias_model->delete($id,$data);
		$data['cb_idcb'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['cb_idcb']);
