...
class App_permiso_roles extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('app_permiso_roles_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('app/view_app_permiso_roles',$data);
	...
	{
		$data = array(
			'pr_rol_id' => $this->input->post('pr_rol_id'),
			'pr_modulo_permiso_id' => $this->input->post('pr_modulo_permiso_id'),
			'pr_permitir' => $this->input->post('pr_permitir'),
			 ); 

		$msn = array(); 

		$result=$this->app_permiso_roles_model->create($data);
		$data['pr_idpr'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['pr_idpr'])) ? $_REQUEST['pr_idpr'] : '';
		$result=$this->app_permiso_roles_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->app_permiso_roles_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->app_permiso_roles_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('pr_idpr');

		$result=$this->app_permiso_roles_model->update($id,$data);
		$data['pr_idpr'] = $id;
	...
	function delete()
	{
		...
		$result=$this->app_permiso_roles_model->delete($id,$data);
		$data['pr_idpr'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['pr_idpr']);
