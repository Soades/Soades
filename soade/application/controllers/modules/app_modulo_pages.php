...
class App_modulo_pages extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('app_modulo_pages_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('app/view_app_modulo_pages',$data);
	...
	{
		$data = array(
			'mp_nombre' => $this->input->post('mp_nombre'),
			'mp_descripcion' => $this->input->post('mp_descripcion'),
			'mp_icono' => $this->input->post('mp_icono'),
			'mp_idmodulo' => $this->input->post('mp_idmodulo'),
			'mp_router' => $this->input->post('mp_router'),
			'mp_orden' => $this->input->post('mp_orden'),
			 ); 

		$msn = array(); 

		$result=$this->app_modulo_pages_model->create($data);
		$data['mp_idmp'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['mp_idmp'])) ? $_REQUEST['mp_idmp'] : '';
		$result=$this->app_modulo_pages_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->app_modulo_pages_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->app_modulo_pages_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('mp_idmp');

		$result=$this->app_modulo_pages_model->update($id,$data);
		$data['mp_idmp'] = $id;
	...
	function delete()
	{
		...
		$result=$this->app_modulo_pages_model->delete($id,$data);
		$data['mp_idmp'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['mp_idmp']);
