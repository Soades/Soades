...
class App_sesiones extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('app_sesiones_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('app/view_app_sesiones',$data);
	...
	{
		$data = array(
			'se_idusuario' => $this->input->post('se_idusuario'),
			'se_estado' => $this->input->post('se_estado'),
			'se_pais' => $this->input->post('se_pais'),
			'se_code' => $this->input->post('se_code'),
			'se_ciudad' => $this->input->post('se_ciudad'),
			'se_detalle' => $this->input->post('se_detalle'),
			'se_fecha' => $this->input->post('se_fecha'),
			 ); 

		$msn = array(); 

		$result=$this->app_sesiones_model->create($data);
		$data['se_idse'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['se_idse'])) ? $_REQUEST['se_idse'] : '';
		$result=$this->app_sesiones_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->app_sesiones_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->app_sesiones_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('se_idse');

		$result=$this->app_sesiones_model->update($id,$data);
		$data['se_idse'] = $id;
	...
	function delete()
	{
		...
		$result=$this->app_sesiones_model->delete($id,$data);
		$data['se_idse'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['se_idse']);
