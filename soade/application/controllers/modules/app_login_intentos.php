...
class App_login_intentos extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('app_login_intentos_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('app/view_app_login_intentos',$data);
	...
	{
		$data = array(
			'li_ip_address' => $this->input->post('li_ip_address'),
			'li_login' => $this->input->post('li_login'),
			'li_time' => $this->input->post('li_time'),
			 ); 

		$msn = array(); 

		$result=$this->app_login_intentos_model->create($data);
		$data['li_idli'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['li_idli'])) ? $_REQUEST['li_idli'] : '';
		$result=$this->app_login_intentos_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->app_login_intentos_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->app_login_intentos_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('li_idli');

		$result=$this->app_login_intentos_model->update($id,$data);
		$data['li_idli'] = $id;
	...
	function delete()
	{
		...
		$result=$this->app_login_intentos_model->delete($id,$data);
		$data['li_idli'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['li_idli']);
