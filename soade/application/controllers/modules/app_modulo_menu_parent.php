...
class App_modulo_menu_parent extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('app_modulo_menu_parent_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('app/view_app_modulo_menu_parent',$data);
	...
	{
		$data = array(
			'mmp_nombre' => $this->input->post('mmp_nombre'),
			'mmp_descripcion' => $this->input->post('mmp_descripcion'),
			'mmp_icono' => $this->input->post('mmp_icono'),
			'mmp_imagen' => $this->input->post('mmp_imagen'),
			'mmp_estado' => $this->input->post('mmp_estado'),
			 ); 

		$msn = array(); 

		$result=$this->app_modulo_menu_parent_model->create($data);
		$data['mmp_idmmp'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['mmp_idmmp'])) ? $_REQUEST['mmp_idmmp'] : '';
		$result=$this->app_modulo_menu_parent_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->app_modulo_menu_parent_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->app_modulo_menu_parent_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('mmp_idmmp');

		$result=$this->app_modulo_menu_parent_model->update($id,$data);
		$data['mmp_idmmp'] = $id;
	...
	function delete()
	{
		...
		$result=$this->app_modulo_menu_parent_model->delete($id,$data);
		$data['mmp_idmmp'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['mmp_idmmp']);
