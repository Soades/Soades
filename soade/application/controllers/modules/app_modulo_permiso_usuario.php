...
class App_modulo_permiso_usuario extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('app_modulo_permiso_usuario_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('app/view_app_modulo_permiso_usuario',$data);
	...
	{
		$data = array(
			'mpu_usuario_id' => $this->input->post('mpu_usuario_id'),
			'mpu_modulo_permiso_id' => $this->input->post('mpu_modulo_permiso_id'),
			'mpu_permitir' => $this->input->post('mpu_permitir'),
			 ); 

		$msn = array(); 

		$result=$this->app_modulo_permiso_usuario_model->create($data);
		$data['mpu_idmpu'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['mpu_idmpu'])) ? $_REQUEST['mpu_idmpu'] : '';
		$result=$this->app_modulo_permiso_usuario_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->app_modulo_permiso_usuario_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->app_modulo_permiso_usuario_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('mpu_idmpu');

		$result=$this->app_modulo_permiso_usuario_model->update($id,$data);
		$data['mpu_idmpu'] = $id;
	...
	function delete()
	{
		...
		$result=$this->app_modulo_permiso_usuario_model->delete($id,$data);
		$data['mpu_idmpu'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem($add['mpu_idmpu']);
