...
var Main = function () {
    ...
    var runNavigationMenu = function () {
        if ($("body").hasClass("single-page") == false) {
            ...
            var url, router, ajaxContainer = $("#ajax-content");
            ...
            if (start.length) {
                ...
                router = start.children("a").attr("data");
                console.log(router + ' : ' + router.split(',')[0])
                if (url != "#") {
            ...
            $('.main-navigation-menu > li a').on('click', function (e) {
                ...
                if ($this.parent().children('ul').hasClass('sub-menu') && (!$('body').hasClass('navigation-small') || !$this.parent().parent().hasClass('main-navigation-menu'))) {
                    ...
                    // url = start.children("a").attr("href");
                    console.log(url)
                    router = $(this).attr("data");
                    console.log(router + ' : ' + router.split(',')[0])
                    if (url != 'javascript:void(0)')
                        if (url != "#") {
                            var div = ' <i class="fa fa-long-arrow-right"></i> ';
                            var titulo2 = (typeof router.split(',')[2] == 'undefined') ? '' : div + '<strong>' + router.split(',')[2] + '</strong>';

                            var sub = '<small>' + router.split(',')[1] + titulo2 + '</small>'
                            //console.log('Ultimo : '+level)
                            $(".page-header").find('h1').html(router.split(',')[0] + sub)
                            ajaxLoader(url, ajaxContainer);
                        }
                    // ajaxLoader(url, ajaxContainer);
                }
