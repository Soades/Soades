...
class Adm_capitulos extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('modules/adm/adm_capitulos_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('modules/adm/view_adm_capitulos',$data);
			$this->AddAudit(array('ca_idca'=>""),ACCESO);
	...
	{
		$data = array(
			'ca_nombre' => $this->input->post('ca_nombre'),
			 ); 

		$msn = array(); 

		$result=$this->adm_capitulos_model->create($data);
		$data['ca_idca'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['ca_idca'])) ? $_REQUEST['ca_idca'] : '';
		$result=$this->adm_capitulos_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->adm_capitulos_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->adm_capitulos_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('ca_idca');

		$result=$this->adm_capitulos_model->update($id,$data);
		$data['ca_idca'] = $id;
	...
	function delete()
	{
		...
		$result=$this->adm_capitulos_model->delete($id,$data);
		$data['ca_idca'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem(isset($add['ca_idca']) ? $add['ca_idca'] : "");
