...
class Adm_ingresos extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('modules/adm/adm_ingresos_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('modules/adm/view_adm_ingresos',$data);
			$this->AddAudit(array('in_idin'=>""),ACCESO);
	...
	function create()
	{
		...
		$result=$this->adm_ingresos_model->create($data);
		$data['in_idin'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['in_idin'])) ? $_REQUEST['in_idin'] : '';
		$result=$this->adm_ingresos_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->adm_ingresos_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->adm_ingresos_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('in_idin');

		$result=$this->adm_ingresos_model->update($id,$data);
		$data['in_idin'] = $id;
	...
	function delete()
	{
		...
		$result=$this->adm_ingresos_model->delete($id,$data);
		$data['in_idin'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem(isset($add['in_idin']) ? $add['in_idin'] : "");
