...
class Adm_resoluciones extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('modules/adm/adm_resoluciones_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('modules/adm/view_adm_resoluciones',$data);
			$this->AddAudit(array('re_idre'=>""),ACCESO);
	...
	{
		$data = array(
			're_numero' => $this->input->post('re_numero'),
			're_fecha' => date("Y-m-d H:i:s"),
			're_concepto' => $this->input->post('re_concepto'),
			 ); 

		$msn = array(); 

		$result=$this->adm_resoluciones_model->create($data);
		$data['re_idre'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['re_idre'])) ? $_REQUEST['re_idre'] : '';
		$result=$this->adm_resoluciones_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->adm_resoluciones_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->adm_resoluciones_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('re_idre');

		$result=$this->adm_resoluciones_model->update($id,$data);
		$data['re_idre'] = $id;
	...
	function delete()
	{
		...
		$result=$this->adm_resoluciones_model->delete($id,$data);
		$data['re_idre'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem(isset($add['re_idre']) ? $add['re_idre'] : "");
