...
class Adm_jornada extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('modules/adm/adm_jornada_model');
	...
	function index()
	{
		if($this->session->userdata('us_usuario'))
		{
			...
			$this->load->view('modules/adm/view_adm_jornada',$data);
			$this->AddAudit(array('jo_idjornada'=>""),ACCESO);
	...
	{
		$data = array(
			'jo_nombre' => $this->input->post('jo_nombre'),
			'jo_orden' => $this->input->post('jo_orden'),
			 ); 

		$msn = array(); 

		$result=$this->adm_jornada_model->create($data);
		$data['jo_idjornada'] = $this->db->insert_id();
	...
	function get_by_id()
	{
		$id =(isset ($_REQUEST['jo_idjornada'])) ? $_REQUEST['jo_idjornada'] : '';
		$result=$this->adm_jornada_model->get_by_id($id);
	...
	function get_all_paginate()
	{
		...
		$data = $this->adm_jornada_model->get_all_paginate($data);
	...
	function get_all()
	{
		...
		$result=$this->adm_jornada_model->get_all();
		echo json_encode($result);
	...
	function update()
	{
		...
		$id =$this->input->post('jo_idjornada');

		$result=$this->adm_jornada_model->update($id,$data);
		$data['jo_idjornada'] = $id;
	...
	function delete()
	{
		...
		$result=$this->adm_jornada_model->delete($id,$data);
		$data['jo_idjornada'] = $id;
	...
	public function AddAudit($add,$permiso)
	{
		...
		$this->audit->setItem(isset($add['jo_idjornada']) ? $add['jo_idjornada'] : "");
