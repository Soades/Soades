...
class Par_ciudades extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('modules/par/par_ciudades_model');
    }
    ...
    function index1() {
        if ($this->session->userdata('us_usuario')) {
            ...
            $this->load->view('modules/par/view_par_ciudades', $data);
            $this->AddAudit(array('ci_idciudad' => ""), ACCESO);
        } else {
    ...
    function create() {
        $data = array(
            'ci_departamento' => $this->input->post('ci_departamento'),
            'ci_nombre' => $this->input->post('ci_nombre'),
            'ci_sync' => $this->input->post('ci_sync'),
        );

        $msn = array();

        $result = $this->par_ciudades_model->create($data);
        $data['ci_idciudad'] = $this->db->insert_id();
     ...
     */
    function get_by_id() {
        $id = (isset($_REQUEST['ci_idciudad'])) ? $_REQUEST['ci_idciudad'] : '';
        $result = $this->par_ciudades_model->get_by_id($id);
    ...
    function get_all_paginate() {
        ...
        $data = $this->par_ciudades_model->get_all_paginate($data);
    ...
    function get_all() {
        ...
        $result = $this->par_ciudades_model->get_all();
        echo json_encode($result);
    }
    ...
    function update() {
        ...
        $id = $this->input->post('ci_idciudad');

        $result = $this->par_ciudades_model->update($id, $data);
        $data['ci_idciudad'] = $id;
    ...
    function delete() {
        ...
        $result = $this->par_ciudades_model->delete($id, $data);
        $data['ci_idciudad'] = $id;
    ...
    public function AddAudit($add, $permiso) {
        ...
        $this->audit->setItem(isset($add['ci_idciudad']) ? $add['ci_idciudad'] : "");
