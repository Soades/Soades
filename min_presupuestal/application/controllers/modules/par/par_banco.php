...
class Par_banco extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('modules/par/par_banco_model');
    }
    ...
    function index1() {
        if ($this->session->userdata('us_usuario')) {
            ...
            $this->load->view('modules/par/view_par_banco', $data);

            $this->AddAudit(array('ba_idbanco' => ""), ACCESO);
        } else {
    ...
    public function AddAudit($add, $permiso) {
        ...
        $this->audit->setItem(isset($add['ba_idbanco']) ? $add['ba_idbanco'] : "");
