...
class Par_paises extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('modules/par/par_paises_model');
    }
    ...
    function index1() {
        if ($this->session->userdata('us_usuario')) {
            ...
            $this->load->view('modules/par/view_par_paises', $data);
            $this->AddAudit(array('pa_idpais' => ""), ACCESO);
...
//            }
    ...
    function save() {
        $data = array(
            ...
            'pa_idpais' => $this->input->post('pa_idpais'),
        );

        $msn = array();
        
        if($this->par_paises_model->existe($data["pa_idpais"])){
            $id = $this->input->post('pa_idpais');           
            $result = $this->par_paises_model->update($id,$data);
            $this->AddAudit($data, EDITAR);
            $msn['result'] = $result;
            $msn['content'] = 'Actualizado con exito !';
            
        }else{
            $result = $this->par_paises_model->create($data);
     ...
     */
    function get_by_id() {
        $id = (isset($_REQUEST['pa_idpais'])) ? $_REQUEST['pa_idpais'] : '';
        $result = $this->par_paises_model->get_by_id($id);
    ...
    function get_all_paginate() {
        ...
        $data = $this->par_paises_model->get_all_paginate($data);
    ...
    function get_all() {
        ...
        $result = $this->par_paises_model->get_all();
        echo json_encode($result);
    }
    ...
    function delete() {
        $data = array(
            'pa_nombre' => $this->input->post('pa_nombre'),
            'pa_sync' => $this->input->post('pa_sync')
        );
        ...
        $result = $this->par_paises_model->delete($id, $data);
        $data['pa_idpais'] = $id;
    ...
    public function AddAudit($add, $permiso) {
        ...
        $this->audit->setItem(isset($add['pa_idpais']) ? $add['pa_idpais'] : "");
