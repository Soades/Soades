...
class Par_departamentos extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('modules/par/par_departamentos_model');
    }
    ...
    function index1() {
        if ($this->session->userdata('us_usuario')) {
            ...
            $this->load->view('modules/par/view_par_departamentos', $data);
            $this->AddAudit(array('de_iddepartamento' => ""), ACCESO);
        } else {
    ...
    function create() {
        $data = array(
            'de_nombre' => $this->input->post('de_nombre'),
            'de_pais' => $this->input->post('de_pais'),
            'de_sync' => $this->input->post('de_sync'),
        );

        $msn = array();

        $result = $this->par_departamentos_model->create($data);
        $data['de_iddepartamento'] = $this->db->insert_id();
     ...
     */
    function get_by_id() {
        $id = (isset($_REQUEST['de_iddepartamento'])) ? $_REQUEST['de_iddepartamento'] : '';
        $result = $this->par_departamentos_model->get_by_id($id);
    ...
    function get_all_paginate() {
        ...
        $data = $this->par_departamentos_model->get_all_paginate($data);
    ...
    function get_all() {
        ...
        $result = $this->par_departamentos_model->get_all();
        echo json_encode($result);
    }
    ...
    function update() {
        ...
        $id = $this->input->post('de_iddepartamento');

        $result = $this->par_departamentos_model->update($id, $data);
        $data['de_iddepartamento'] = $id;
    ...
    function delete() {
        ...
        $result = $this->par_departamentos_model->delete($id, $data);
        $data['de_iddepartamento'] = $id;
    ...
    public function AddAudit($add, $permiso) {
        ...
        $this->audit->setItem(isset($add['de_iddepartamento']) ? $add['de_iddepartamento'] : "");
