...
class Par_tipo_identificacion extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('modules/par/par_tipo_identificacion_model');
    }
    ...
    function index1() {
        if ($this->session->userdata('us_usuario')) {
            ...
            $this->load->view('modules/par/view_par_tipo_identificacion', $data);
            $this->AddAudit(array('ti_idti' => ""), ACCESO);
        } else {
...
//            }
    ...
    function create() {
        $data = array(
            'ti_nombre' => $this->input->post('ti_nombre'),
        );

        $msn = array();

        $result = $this->par_tipo_identificacion_model->create($data);
        $data['ti_idti'] = $this->db->insert_id();
     ...
     */
    function get_by_id() {
        $id = (isset($_REQUEST['ti_idti'])) ? $_REQUEST['ti_idti'] : '';
        $result = $this->par_tipo_identificacion_model->get_by_id($id);
    ...
    function get_all_paginate() {
        ...
        $data = $this->par_tipo_identificacion_model->get_all_paginate($data);
    ...
    function get_all() {
        ...
        $result = $this->par_tipo_identificacion_model->get_all();
        echo json_encode($result);
    }
    ...
    function update() {
        ...
        $id = $this->input->post('ti_idti');

        $result = $this->par_tipo_identificacion_model->update($id, $data);
        $data['ti_idti'] = $id;
    ...
    function delete() {
        ...
        $result = $this->par_tipo_identificacion_model->delete($id, $data);
        $data['ti_idti'] = $id;
    ...
    public function AddAudit($add, $permiso) {
        ...
        $this->audit->setItem(isset($add['ti_idti']) ? $add['ti_idti'] : "");
