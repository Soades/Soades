...
class Par_departamentos_model extends CI_Model
{
	...
	function create($item)
	{
		$data = array(
			...
			'de_sync' => $item['de_sync']
			 ); 

		 return $this->db->insert('par_departamentos', $data);
	}
	...
	function get_all_paginate($vars)
	{
		...
		if($search != ''){
			...
			$search_op  .=  ' de_pais like "%'.$search.'%" or ';
			$search_op  .=  ' de_sync like "%'.$search.'%"  ';
			$search = $search_op;
		}
		...
		$sql .= "<table id='app_roles_table'  class='table table-bordered kgrid tablesorter-materialize'>
			...
			<tr>
				...
				<th><a class='k-link' href='#'> de_iddepartamento</a></th> 
				<th><a class='k-link' href='#'> de_nombre</a></th> 
				<th><a class='k-link' href='#'> de_pais</a></th> 
				<th><a class='k-link' href='#'> de_sync</a></th> 
			</tr>
		...
		foreach ($result_set->result() as $row) {
				...
				$sql .= '<tr>';
				$sql .= '<td class="x-grid-cell-special" data="'.$row->de_iddepartamento.'" >' . $con . '</td>';
				$sql .= '<td '.$cls.' data="'.$row->de_iddepartamento.'" >' . $row->de_iddepartamento . '</td>';
				$sql .= '<td '.$cls.' data="'.$row->de_iddepartamento.'" >' . $row->de_nombre . '</td>';
				$sql .= '<td '.$cls.' data="'.$row->de_iddepartamento.'" >' . $row->de_pais . '</td>';
				$sql .= '<td '.$cls.' data="'.$row->de_iddepartamento.'" >' . $row->de_sync . '</td>';
				$sql .= '<tr>';
		}
	...
	function update($id, $item)
	{
		...
		$this->db->where('de_iddepartamento', $id);
		return $this->db->update('par_departamentos', $data);
	}

	function delete($id)
	{
		$this->db->where('de_iddepartamento', $id);
		 return $this->db->delete('par_departamentos');
	}
}