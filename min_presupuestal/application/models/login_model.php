...
class Login_model extends CI_Model {
    ...
    public function login($anio, $codigo, $identity, $password, $remember = FALSE) {
        ...
        //                          us_usuario,us_clave, us_email, us_idusuario id, us_clave, us_active, us_ultimo_acceso,ro_idroles,ro_nombre
        //                          FROM " . $this->tables['users'] . "
        //                          INNER JOIN app_roles ON app_roles.ro_idroles = us_roles
        //                          WHERE us_usuario = '".$identity."'
        //                          order by us_idusuario desc
        //                          limit 1
        //                        ");
        ...
        if ($query->num_rows() === 1) {
            $user = $query->row();
            if (!$user->in_estado) {
                $session["message"] = self::$lenguaje->line("INSTITUTE_INACTIVE");//"Lo sentimos, la institucion esta inactiva !";
                $session["session"] = FALSE;
                return $session;
            } elseif (md5($password) == $user->us_clave) {
                if ($user->us_active == 0) {
                    $session["message"] = self::$lenguaje->line("USERS_INACTIVE");//"Lo sentimos, el usuario esta inactivo";
                    $session["session"] = FALSE;
                    return $session;
                }
                ...
                $session["message"] = self::$lenguaje->line("ACCESS_SUCCEFULL");//"Acceso exitoso";
        ...
        $session["message"] = self::$lenguaje->line("ERROR_LOGIN");//"Error al logearse";
        $session["session"] = FALSE;
        return $session;
    }
