...
class Desktop_model extends CI_Model {
    ...
    public function Get_menu() {
        ...
        $menu = '<ul>';
        $menu .= '<li class="side-nav-item active">
                            <a href="index.html">
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $news) {
                ...
                $menu .= '<li class="side-nav-item" >
                              <a href="#MENU_' . $news->mmp_nombre . '">
                              <span title="' . ($news->level > 0 ? $news->level . ' items' : '' ) . '" class="badge">' . ($news->level > 0 ? $news->level : '' ) . '</span>                             
                              
                              <div class="nav-item-icon icon" >
                              <img class="" style="width: 100%;height: 100%;" src="'.BASE.'public/template/ministerio/images/parent/'.$news->mmp_imagen.'" >   
                              </div>
                              <span class="title">' . $news->mmp_descripcion . '</span>' . ($news->level > 0 ? '<i class="nav-item-caret"></i>' : '') . '
                              </a>';
                if ($news->level > 0) {
    ...
    public function GetModulos($idmodulo, $padre_nom, $padre) {
        ...
        $mt = '';
        if ($query->num_rows() > 0) {
            $mt .= '<ul id="MENU_' . $padre_nom . '" class="side-nav-child">';

            $mt .= '<li class="side-nav-item-heading">
                                            <a href="#MENU_' . $padre_nom . '" class="side-nav-back">
    ...
    function GetUlMultilevel($idmodulo, $padre, $hijo_nombre, $hijo) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $mt .= '<li class="side-nav-item-heading">
                                            <a  href="#MENU_HIJO_' . $hijo_nombre . '" class="side-nav-back">
                                                <i class="nav-item-caret"></i>
                                               ' . $padre . '
                                            </a>
                                        </li>';
            ...
            foreach ($query->result() as $news) {
                ...
                $mt .= '<a class="cl_data_router" data-router='.$news->mp_router.' data-modulo-name=' . $news->mp_nombre . ' data=' . $news->mp_nombre. ' href="#"> ';
